...
document.getElementById("searchInput").addEventListener("input", function () {
  ...
  rows.forEach(function (row) {
    ...
    var nombreEquipo = row
      .querySelector("td:nth-child(2)")
      .innerText.toLowerCase();

    if (
      cj.endsWith("1") &&
      (cj.includes(searchTerm) || nombreEquipo.includes(searchTerm))
    ) {
      row.style.display = "";
    } else {
...
document
  .getElementById("exportarButton")
  .addEventListener("click", function () {
    const doc = new jspdf.jsPDF();

    // Configura el tipo de fuente a Arial, el estilo normal y el tamaño a 12
    doc.setFont("Arial");
    doc.setFontSize(12);

    // Añade el texto "Hola Mundo" al documento
